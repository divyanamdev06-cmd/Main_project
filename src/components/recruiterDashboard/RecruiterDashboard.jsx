...
export default function RecruiterDashboard() {
  ...
  const initial = (user?.name || user?.companyName || "R").charAt(0).toUpperCase();
      ...
      >
        <div>
          ...
          <div className="card mb-6 border-amber-100/80 bg-white/90 p-4 text-center shadow-sm ring-1 ring-amber-100/60">
            ...
            <h1 className="mt-3 text-base font-bold tracking-tight text-gray-900">Recruiter</h1>
            <p className="mt-0.5 text-xs font-medium text-gray-500">
              {user?.companyName || user?.name || "JobNest"}
            </p>
            <p className="mt-1 text-[10px] uppercase tracking-wider text-amber-800/80">{role}</p>
          </div>
          ...
          <nav className="flex flex-col gap-1 text-sm font-semibold">
            ...
            >
              <NavItem icon={<Briefcase size={18} strokeWidth={2} />} label="My job posts" />
            </NavLink>
            ...
            >
              <NavItem icon={<User size={18} strokeWidth={2} />} label="Company profile" />
            </NavLink>
          </nav>
        </div>
        ...
        <header className="fixed top-0 right-0 left-0 z-40 flex h-14 shrink-0 items-center justify-between gap-3 border-b border-amber-100/80 bg-white/90 px-3 shadow-sm backdrop-blur-md sm:h-16 sm:px-5 md:left-64">
          <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-amber-700/90 sm:text-xs">JobNest</p>
              <h2 className="truncate text-base font-bold text-gray-900 sm:text-lg">Recruiter workspace</h2>
            </div>
          </div>
